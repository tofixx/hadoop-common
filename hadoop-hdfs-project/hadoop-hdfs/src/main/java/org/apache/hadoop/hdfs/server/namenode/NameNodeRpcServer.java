...
class NameNodeRpcServer implements NamenodeProtocols {
    ...
    public LocatedBlocks getBlockLocations(String src,
        ...
        if (NamenodeWebHdfsMethods.isWebHdfsInvocation()) {
            locatedBlocks = this.energyBaseDataNodeFilter.filterBlockLocations(
                ...
                NamenodeWebHdfsMethods.getRemoteAddress(), nn.conf.get("BlockFilterStrategy"));
        } else if (Server.isRpcInvocation()) {
            locatedBlocks = this.energyBaseDataNodeFilter.filterBlockLocations(
                locatedBlocks, src, Server.getRemoteUser().getUserName(),
                Server.getRemoteAddress(), nn.conf.get("BlockFilterStrategy"));
        } else {
     ...
     *
     * @param bandwidth Balancer bandwidth in bytes per second for all
     * datanodes.
     * @throws IOException
     */
    @Override // ClientProtocol
